...
from tools.serializers import GenderSerializer
...
class ClientTypeGenderCreateAPIView(APIView):
    ...
    def post(self, request):
       try:
            ...
            serializer = GenderSerializer(data=request.data, context={"db_name": DB_NAME})
...
class ClientTypeGenderListAPIView(APIView):
    
    def get(self, request):
        try:
            gender = ClientTypeGender.objects.using(DB_NAME).filter(is_void=False)
            serializer = GenderSerializer(gender, many=True, context={"db_name": DB_NAME})
            response_msg = {
                globalparameters.RESULT_CODE: globalparameters.RESULT_CODE_SUCCESS,
                globalparameters.RESULT_DESCRIPTION: globalparameters.RESULT_DESCRIPTION_SUCCESS,
        ...
        except ClientTypeGender.DoesNotExist as exc:
            logger.error(str(exc), exc_info=True)
            error_msg = {
