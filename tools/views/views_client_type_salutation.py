...
from tools.serializers import SalutationSerializer
...
class SalutationCreateAPIView(APIView):
    ...
    def post(self, request):
       try:
            ...
            serializer = SalutationSerializer(data=request.data, context={"db_name": DB_NAME})
...
class SalutationListAPIView(APIView):
    
    def get(self, request):
        try:
            salutation = Salutation.objects.using(DB_NAME).filter(is_void=False)
            serializer = SalutationSerializer(salutation, many=True, context={"db_name": DB_NAME})
            response_msg = {
                globalparameters.RESULT_CODE: globalparameters.RESULT_CODE_SUCCESS,
                globalparameters.RESULT_DESCRIPTION: globalparameters.RESULT_DESCRIPTION_SUCCESS,
