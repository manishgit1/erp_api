...
from tools.serializers import PaymentSchemeSerializer
...
class LoanPaymentSchemeCreateAPIView(APIView):
    ...
    def post(self, request):
       try:
            ...
            serializer = PaymentSchemeSerializer(data=request.data, context={"db_name": DB_NAME})
...
class LoanPaymentSchemeListAPIView(APIView):
    
    def get(self, request):
        try:
            nationality = LoanPaymentScheme.objects.using(DB_NAME).filter(is_void=False)
            serializer = PaymentSchemeSerializer(nationality, many=True, context={"db_name": DB_NAME})
            response_msg = {
                globalparameters.RESULT_CODE: globalparameters.RESULT_CODE_SUCCESS,
                globalparameters.RESULT_DESCRIPTION: globalparameters.RESULT_DESCRIPTION_SUCCESS,
        ...
        except LoanPaymentScheme.DoesNotExist as exc:
            logger.error(str(exc), exc_info=True)
            error_msg = {
