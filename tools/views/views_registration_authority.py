...
from tools.serializers import RegistrationAuthoritySerializer
...
class RegistrationAuthorityCreateAPIView(APIView):
    ...
    def post(self, request):
       try:
            ...
            serializer = RegistrationAuthoritySerializer(data=request.data, context={"db_name": DB_NAME})
...
class RegistrationAuthorityListAPIView(APIView):
    
    def get(self, request):
        try:
            registration_authority = RegistrationAuthority.objects.using(DB_NAME).filter(is_void=False)
            serializer = RegistrationAuthoritySerializer(registration_authority, many=True, context={"db_name": DB_NAME})
            response_msg = {
                globalparameters.RESULT_CODE: globalparameters.RESULT_CODE_SUCCESS,
                globalparameters.RESULT_DESCRIPTION: globalparameters.RESULT_DESCRIPTION_SUCCESS,
        ...
        except RegistrationAuthority.DoesNotExist as exc:
            logger.error(str(exc), exc_info=True)
            error_msg = {
