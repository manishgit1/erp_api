...
from tools.serializers import EducationSerializer
...
class EducationCreateAPIView(APIView):
    ...
    def post(self, request):
       try:
            ...
            serializer = EducationSerializer(data=request.data, context={"db_name": DB_NAME})
...
class EducationListAPIView(APIView):
    ...
    def get(self, request):
        try:
            education = Education.objects.using(DB_NAME).filter(is_void=False)
            serializer = EducationSerializer(education, many=True, context={"db_name": DB_NAME})
            response_msg = {
                globalparameters.RESULT_CODE: globalparameters.RESULT_CODE_SUCCESS,
                globalparameters.RESULT_DESCRIPTION: globalparameters.RESULT_DESCRIPTION_SUCCESS,
